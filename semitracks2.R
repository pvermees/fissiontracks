...
invert <- function(l,confined=FALSE,ncomp=5){
    ...
    if (confined) fit <- optim(p,cmisfit,M=M,S=S,l=l)
    else fit <- optim(p,smisfit,M=M,S=S,l=l)
    P <- p2P(fit$par)
...
# dat: fission track length data generated by the read.data function
# ncomp: the number of Normal components to fit
invert2 <- function(l,confined=FALSE,ncomp=5){
    pldM <- rep(0,2*ncomp-2) # initialise with equal logratios
    if (confined) fit <- optim(p,cmisfit,M=M,S=S,l=l)
    else fit <- optim(pldM,smisfit2,l=l)
    P <- p2P(fit$par[1:(ncomp-1)])
    M <- ldM2M(fit$par[ncomp:(2*ncomp-2)])
    list(l=l,P=P,M=M,S=S)
}
...
plotModel <- function(m,d=NULL){
    ...
    if (is.null(d)){
        ...
        if (d$confined){
            lines(m$l,m$fc)
            title(smisfit(P2p(m$P),m$M,m$S,d$l))
...
# inverse logratio transformation
ldM2M <- function(ldM,mM=4,MM=16){
    nn <- length(ldM)
    dM <- c(exp(ldM),1)/(sum(exp(ldM))+1)
    mM + (MM-mM)*cumsum(dM)[1:(nn-1)]
}
