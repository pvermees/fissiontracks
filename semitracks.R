...
read.data <- function(fname,confined=FALSE,cutoff=4,header=TRUE,skip=0,cols=c(5,9)){
...
simulate <- function(fm,n=1000,confined=FALSE){
    if (confined) dens <- fm$fc
    else dens <- fm$fs
...
smisfit <- function(pms,l,mM=4,MM=16,dM=2){
    ...
    dM <- 2
    num.of.short.tracks <- sum(l>=cutoff & l<(cutoff+dM))
    num.uncounted <- num.of.short.tracks*cutoff/dM
